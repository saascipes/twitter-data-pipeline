...
2. RabbitMQ - must have the stomp protocol enabled and be publicly accessible
...
3. Import and configure SaaSGlue jobs
    ...
    2. Import SaaSGlue jobs
        - This will import 2 jobs and the associated tasks, steps and scripts into your SaaSGlue team workspace.
            1. Tweet Stream Analyzer
                - Tasks
                    ...
                    - Analyze tweets - thiis task will analyze the tweets, extract the major topics and publish them to a message queue
            ...
            2. Tweet Stream Analyzer Local - this job performs the same functions as the first job except that the "Stream tweets" task runs on your compute (any linux or mac machine) running the SaaSGlue agent rather than as a lambda function
    ...
    3. Configure SaaSGlue jobs
        - Configure runtime variables - perform the following steps for the two jobs
            - Click "Designer" in the menu bar
            - Click the job name ("Tweet Stream Analyzer" or "Tweet Stream Analyzer Local")
...
6. Create vue config file
    ...
    ```
    - If you want to run this in a production environment you'll need to create a similar file named ".env.production"
